...
void SeekBar::createSeekBar(char* frontResource,char* backResource,CCPoint point,float progress)
{
    ...
    m_seekBarRect = CCRectMake(point.x - seekBarSize.width * 0.5, winSize.height - (point.y + seekBarSize.height * 0.5), seekBarSize.width, seekBarSize.height);
...
float SeekBar::redrawSeekBar(CCPoint touchPoint)
{
    ...
    seekBar->setPosition(ccp(m_seekBarRect.getMidX() - (SeekBarWidth - diffX) * 0.5,winSize.height - m_seekBarRect.getMidY()));
...
void SeekBar::setSeekBarListener(SeekBarListener *listener,int tag)
{
    m_listener = listener;
    m_listenerTag = tag;
}
...
bool SeekBar::ccTouchBegan(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent)
{
    ...
    if(m_seekBarRect.containsPoint(touchPoint))
    {
        ...
        if(m_listener != NULL)
        {
            m_listener->SeekStart(progress,m_listenerTag);
        }
...
void SeekBar::ccTouchMoved(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent)
{
    ...
    if(m_listener != NULL)
    {
        m_listener->SeekMove(progress,m_listenerTag);
    }
...
void SeekBar::ccTouchEnded(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent)
{
    ...
    if(m_listener != NULL)
    {
        m_listener->SeekEnd(progress,m_listenerTag);
    }
}